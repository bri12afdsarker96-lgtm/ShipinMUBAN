...
**自查结果**：`tsc` 通过、`gui:build` 通过、回归 **36/0**（新增 8 条越界/非法拒绝 + 合法落盘 + 读回 + 缺字段 400 等用例）；另手工验证“手改配置文件注入 `../../etc` → GET 回落 `editor`、解析路径仍在 `out/` 内”。渲染集成用例仍由 CI（`RUN_RENDER_TESTS=1`）在装好浏览器后真跑，本环境网络策略下不静默跳过、只在无浏览器时提示。
