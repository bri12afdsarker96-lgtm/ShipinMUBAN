...
## 6c. v0.3 独立复核归档：素材库管理与路径安全

v0.3 远端提交 `b2a41f6` 已完成独立复核，本轮重点检查新增的素材删除/重命名接口。该能力是新的高风险面：如果边界校验缺失，可能演变为任意文件删除或任意文件改名。

**复核结论：通过，无需修改。**

| 检查项 | 结论 |
| --- | --- |
| 单提交范围 | 只改 10 个文件，未碰核心渲染文件，未误提交 `release/` 等二进制产物 |
| 删除/重命名路径遍历 | `resolveMutableAsset` 拒绝 `..`、反斜杠、URL、`data:`、`blob:`，并强制 `clean === <分类>/<文件名>`；子目录、跨分类、遍历路径均无法通过 |
| 扩展名与文件名 | 重命名使用 `safeSlug` 清理新名，并强制保留原扩展名；`../../x.html` 只能落为安全的同类原扩展名文件 |
| 边界兜底 | 最终路径继续通过 `withinBase` 校验；同名目标不覆盖，会追加后缀 |
| 设置页目录边界 | 当前 `/api/settings` 只读，无用户可写目录进入静态托管或路径解析，暂不存在任意目录暴露风险 |
| 既有安全加固 | Host 校验、`Connection: close`、上传 `safeSlug`、路由 `await` 均保留 |
| 回归测试 | `tsc`、`gui:build` 通过；完整回归 28/0，通过越界删除拒绝、危险重命名清理、素材删除仅限清单内目标等新增用例 |
| UI 验收 | 素材库多页面渲染正常，搜索、筛选、缩略图、复制、改名、删除入口齐全 |

本轮还新增并采纳 `docs/v0.3-review-checklist.md`，把删除/重命名路径安全、Electron/Web 边界、产品验收与仓库卫生纳入后续复核清单。后续若设置页开放“素材目录/输出目录可写配置”，需要重新复核目录边界，确保不会把项目外任意路径暴露给静态服务或素材操作接口。
...
## 8. v0.4：设置页可写输出目录 + 边界校验

承接第 6c/7 节留下的“若设置页开放可写目录须重新复核目录边界”，本轮把设置页从只读升级为**受控可写**，并把安全边界作为首要设计约束。

**范围**：只放开“编辑器单片渲染输出子目录”，严格锁定在 `out/` 根内；批量任务仍固定归档到 `out/editor/<jobId>/`，行为不变、零回归。

**边界校验（server.mjs）**：

| 层次 | 做法 |
| --- | --- |
| 硬拒危险形态 | `parseOutputSubdir` 直接拒绝 `..`、反斜杠、绝对路径、`盘符:`、`~`、控制字符 |
| 逐段白名单 | 按 `/` 拆段，每段必须匹配 `[A-Za-z0-9一-龥_-]{1,40}`，层级 ≤4，非法段返回 400 明确文案 |
| Windows 保留名 | 拒绝 `CON`、`AUX`、`NUL`、`PRN`、`COM1`-`COM9`、`LPT1`-`LPT9`，避免 Windows 下目录创建或渲染输出异常 |
| 边界兜底 | 拼接后再过 `withinBase(outDir)`，越界即拒（纵深防御） |
| 读取二次校验 | `currentOutputSubdir` 读取持久化值时**重新过一遍校验**——即便有人手改 `config/editor-settings.json` 注入 `../../etc`，也回落默认 `editor`，持久化值绝不被无条件信任 |

**持久化**：写入 `config/editor-settings.json`（已加入 `.gitignore`，属本地运行时配置，不入库）。

**接口**：`GET /api/settings` 增加 `writable.outputSubdir`（当前值/默认值/解析路径/根/层级上限/文案）；`POST /api/settings` 校验并保存，非法输入统一 400。`handleRender`/`handleRenders` 改用当前有效目录与对应 URL 前缀。

**前端**：设置页“目录设置”内新增可编辑子目录表单（`out/` 前缀 + 输入框 + 保存 + 解析路径回显 + 成功/错误提示）。

**独立复核补充**：手动攻击测试发现 `safe/CON` 会被原实现接受。它不会越出 `out/`，但在 Windows 上属于保留设备名，可能导致目录创建或渲染输出异常。本轮已加拒绝规则，并补 `CON`、`safe/AUX`、`LPT1` 回归用例。

**自查结果**：`tsc` 通过、`gui:build` 通过、回归 **44/0**（新增越界/非法拒绝、Windows 保留名拒绝、合法落盘、读回、缺字段 400 等用例，并包含真实浏览器渲染）；另手工验证“手改配置文件注入 `../../etc` → GET 回落 `editor`、解析路径仍在 `out/` 内”。

**未覆盖/留待验收**：Electron 桌面端“打开文件夹/系统播放器”仍属客户端能力，需在 Windows 侧验收，本无头环境无法验证。
