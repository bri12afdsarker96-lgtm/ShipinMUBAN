...
| 阶段 4A 全画面素材替换 | 已完成 | 快闪书封可按行或逐本上传/预览，快闪背景、主书背景、开场视频/背景、音乐均可上传/配置；`npm test` 15/0 通过 |
...
| 阶段 5C 素材库管理与可视化选择 | 已完成 | v0.3.0：素材搜索/筛选/改名/删除、可视化素材选择、任务中心 Web 降级、设置目录信息；完整回归 28/0 |
...
- 2026-07-19：完成阶段 4A 第二版，落实“凡是视频中展现出来的素材都需要可替换”。
  快闪书单每本封面、快闪段背景、主书背景、开场视频、生成式开场背景、开场角标、背景音乐均可在编辑器配置；
  上传接口扩展到 `covers/backgrounds/introVideos/audio`。验证：`node --check` 通过；
  `npx.cmd tsc --noEmit` 通过；`npm.cmd run gui:build` 通过；`npm.cmd test` 15/0 通过；
  接口冒烟确认新素材路径可被批量配置接收。
...
- 2026-07-19：根据复核补齐快闪书单封面逐本替换入口。编辑器在保留批量文本和按行上传的同时，
  为每本快闪书展示封面预览、封面路径输入和单独上传按钮，避免用户只能通过第三列隐式替换。
- 2026-07-19：启动阶段 5A 桌面封装，软件命名为“水星视频模板”。新增 Electron 主进程入口、
  `electron-builder` NSIS 引导式安装包配置，并将 GUI 外层改为参考“水星下载”的桌面工具界面。
  当前 `npm.cmd run gui:build` 通过；初次 Electron 运行时下载在本机多次超时，后续已改用镜像打通。
...
- 2026-07-19：完成 v0.2.1 客户端信息架构迭代。`/api/assets` 改为扫描真实素材目录；新增独立素材库、
  ...
  `http://127.0.0.1:43110/api/health` 返回 `{"ok":true}`。
- 2026-07-20：完成 v0.3.0 素材库管理与可视化选择迭代。新增素材改名/删除接口及页面内改名、二次确认删除；
  主制作页素材位改为可视化选择弹层，快闪书单逐本封面入口保留；任务中心提供播放/下载/复制路径的 Web 降级；
  设置页展示素材目录与输出目录。验证：`node --check`、`npx.cmd tsc --noEmit`、`npm.cmd run gui:build`
  通过；`RUN_RENDER_TESTS=1 npm.cmd test` 28/0 通过；浏览器实测素材搜索/筛选、改名/删除、主界面选择、任务中心、
  设置页通过。`npm.cmd run dist:win` 已生成 `book-intro-maker/release/水星视频模板-Setup-0.3.0.exe`，大小
  134,892,118 bytes，SHA256 `0527C27D1D6863FB57E16CCDC94FE6E95CCEE0D31EEB76CF20919AF29359B659`。
