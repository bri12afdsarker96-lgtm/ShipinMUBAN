...
用户复核指出三处产品问题：可上传素材少、通用操作里的“素材替换”无效、“任务列表”仍显示批量渲染队列。本阶段把桌面端从“网页表单包壳”调整为更接近软件的模块结构。

| 项目 | 结果 |
| --- | --- |
| 版本号 | `0.2.1` |
| 素材库 API | `/api/assets` 改为扫描真实 `public/covers`、`backgrounds`、`intro-videos`、`audio` 目录，上传后可出现在素材库清单 |
| 素材库页面 | 新增独立“素材库”，支持封面、背景、开场视频、音乐分类展示、批量上传、预览、复制路径 |
| 主编辑页 | 背景音乐、快闪背景、主书封面、主书背景、快闪书单逐本封面、开场视频、开场背景均改为从素材库下拉选择，上传作为补充入口 |
| 任务中心 | 新增独立“任务中心”，展示最近生成视频与批量任务记录 |
| 设置页 | 新增“设置 / 高级配置”，导出 JSON 从主编辑页移出，放到设置页 |
| 导航纠偏 | “素材替换”不再假跳转编辑页；“任务列表”不再打开批量生产页 |
...
- `npx.cmd tsc --noEmit` 通过。
- `npm.cmd run gui:build` 通过。
- `npm.cmd test` 通过 18 项，新增覆盖“上传后出现在素材库清单”。
